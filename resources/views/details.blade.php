...
<head>
    <title>মামলার তথ্য | এলএসটি, মৌলভীবাজার</title>
            ...
            <div class="row mx-1">
              <div class="col-sm-6 p-2 my-3" style="background: #057a8d;">
                <div class="p-2" style="border: 1px solid #ffffcc;">
                  <h5 class="mb-0 text-white">মামলার তথ্য দেখুন</h5>
                </div>
              </div>
          ...
          <div class="card">
            ...
            <table class="table table-striped table-bordered table-responsive-sm text-center mb-0">
              <thead class="" style="">
                ...
                <th class="bg-success font-weight-normal text-white">বর্তমান তারিখ</th>
           ...
           @foreach ($Details as $i => $dt)
               <tr class="">
                  <td>{{ trans2bn($i+1) }}</td>
                  {{-- <td>{{ en2bnNumber($dt->caseid) }}</td> --}}
                  <td>
                    @if ($dt->c_date == '1970-01-01' || $dt->c_date == '0000-00-00' || $dt->c_date == NULL)
                        {{ 'ফাইলিং' }}
                    @else
                        {{ trans2bn(str_replace('-', '/', date('d-m-Y', strtotime($dt->c_date)))) }}
                    @endif
                </td>
                  <td>{{ $dt->c_for }}</td>
                  <td>
                    @if ($dt->n_date == '1970-01-01' || $dt->n_date == '0000-00-00' || $dt->n_date == NULL)
                        {{ '' }}
                    @else
                        {{ trans2bn(str_replace('-', '/', date('d-m-Y', strtotime($dt->n_date)))) }}
                    @endif
                </td>
                  <td>{{ $dt->n_for }}</td>
                  <td>{{ $dt->remark }}</td>
